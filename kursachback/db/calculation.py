...
class TourParser:
    ...
    def adding_in_the_list_new_coast_tour(self, route):
        a = + route.costofroute
        self.coast.append(a)
        self.count_tours.append(0)

    def get_coast_tours(self, tours, routes):
        if len(routes) > 1 and len(tours) > 1:
            self.parsing_tours_and_routes(tours, routes)
            return [self.coast, self.count_tours]
            ...
            for route in routes:
                self.adding_in_the_list_new_coast_tour(route)
            return [self.coast, self.count_tours]
